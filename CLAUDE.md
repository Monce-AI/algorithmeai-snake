...
```bash
# Install Cython + build .so
pip install "Cython>=3.0"
python setup.py build_ext --inplace

# Verify it loaded
python -c "from algorithmeai._accel import apply_literal_fast; print('Cython active')"
```

**What Cython accelerates:** `apply_literal` (all 30 literal types including LEV, ENT, CFC with C-level helpers), `apply_clause`, `traverse_chain`, `get_lookalikes`, batch operations. Expect **~3x speedup** on both training and inference. The `.pyx` includes C-level `_levenshtein_c` (exact DP for ≤32 chars, O(n) bag-of-chars for longer), `_entropy_c`, `_hex_ratio_c`, `_repeat_period_score_c`.

**Important:** After modifying `_accel.pyx`, always rebuild with `python setup.py build_ext --inplace`. The `.c` file is auto-generated by Cython and should not be committed. The `.so` is platform-specific and gitignored.

Without Cython, pure Python runs identically. The `_HAS_ACCEL` flag gates all fast paths.
...
- **Dataset-specific profiles**: 8 `.snake` files in `profiles/` with empirical annotations and rationale. Each tested against original on the corresponding dataset.
- **Snake vs RF/GB**: On same data with zero preprocessing — Snake 0.999 AUROC on Breast Cancer (RF: 0.997, GB: 0.995). Ties on Iris/Wine. Classic Titanic with Names: 0.924 AUROC / 87.2% accuracy (cryptographic profile, +3.8pp over original).
- **Meta removed**: `algorithmeai/meta.py` and `tests/test_meta.py` deleted. `__init__.py` no longer exports Meta.
- **Linguistic deprecated**: Never won empirically, blocks on long text. Pure NLP out of Snake's scope.
- **Cython bool-safe**: `str(field)` casts replace `<str>field` assertions in `_accel.pyx`, handling bool/None values.
